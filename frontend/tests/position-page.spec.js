...
const interviewFlowPayload = {
  interviewFlow: {
    ...
    interviewFlow: {
      ...
      interviewSteps: [
        { id: 101, interviewFlowId: 1, interviewTypeId: 1, name: 'Screening', orderIndex: 1 },
        { id: 102, interviewFlowId: 1, interviewTypeId: 2, name: 'Technical', orderIndex: 2 },
        { id: 103, interviewFlowId: 1, interviewTypeId: 3, name: 'Final', orderIndex: 3 },
      ],
    },
  },
};
...
const waitForPageLoad = async (page) => {
  ...
  await expect(page.getByRole('heading', { name: /Posiciones/i })).toBeVisible();
};
...
test.describe('Position page end-to-end flow', () => {
    ...
    await page.route('**/candidates/1', async (route) => {
      ...
      if (request.method() === 'PUT') {
        candidatesPayload = candidatesPayload.map((candidate) =>
          candidate.id === 1 ? { ...candidate, currentInterviewStep: 'Technical' } : candidate
        );
      }
  ...
  test('navigates to the Position page and loads available positions', async ({ page }) => {
    await expect(page.getByRole('button', { name: /Ver proceso/i }).first()).toBeVisible();
    await expect(page.getByText(/Manager:/i).first()).toBeVisible();
  });

  test('loads interview stages, renders candidates, and supports drag and drop between stages', async ({ page }) => {
  ...
  test('shows accessible regions and supports back navigation', async ({ page }) => {
    await page.getByRole('button', { name: /Ver proceso/i }).first().click();
  ...
  test('renders a responsive layout on a smaller viewport', async ({ page }) => {
